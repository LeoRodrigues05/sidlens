...
class AmazonReviews2014(AbstractDataset):
    ...
    def _download_and_process_raw(self):
        ...
        # Check if processed files already exist (skip download if so). Compute
        # nodes have no outbound network, so the snap.stanford.edu fetch below
        # cannot succeed; the shipped cache already carries the processed split.
        ...
        if os.path.exists(seq_file) and os.path.exists(id_mapping_file):
            self.log('[DATASET] Processed files found, skipping download...')
