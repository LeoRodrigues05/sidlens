...
class AbstractDataset:
    ...
    def _leave_one_out(self):
        ...
        Supports n_target_items: each split needs at least n_target_items + 1 items.
        ...
        """
        n_target = self.config.get('n_target_items', 1)
        datasets = {'train': {'user': [], 'item_seq': []},
                    'val': {'user': [], 'item_seq': []},
                    'test': {'user': [], 'item_seq': []}}
        for user in self.all_item_seqs:
            seq = self.all_item_seqs[user]
            # test: 需要至少 n_target + 1 个 item (n_target个target + 至少1个history)
            if len(seq) >= n_target + 1:
                datasets['test']['user'].append(user)
                datasets['test']['item_seq'].append(seq)
            # val: 去掉最后1个后还需要 n_target + 1
            if len(seq) - 1 >= n_target + 1:
                datasets['val']['user'].append(user)
                datasets['val']['item_seq'].append(seq[:-1])
            # train: 去掉最后2个后还需要 n_target + 1
            if len(seq) - 2 >= n_target + 1:
    ...
    def _sliding_train(self, min_hist=2, max_hist=50):
        ...
        支持 n_target_items: 每个样本需要 n_target 个 target + 至少 min_hist 个 history。
        ...
        for user, seq in self.all_item_seqs.items():
            ...
            # test: 需要至少 n_target + 1 个 item
            if L >= n_target + 1:
                datasets['test']['user'].append(user)
                datasets['test']['item_seq'].append(seq)

            # val: 去掉最后 1 个后还需 n_target + 1
            if L - 1 >= n_target + 1:
                datasets['val']['user'].append(user)
                datasets['val']['item_seq'].append(seq[:-1])

            # train: 滑窗，每个窗口长度至少 n_target + min_hist
            min_window = n_target + min_hist  # 最小窗口长度
            if L > 2:
                # 训练label最远限制到L-3，保证不与val/test target重叠
                hi_max = min(L - 3, max_hist)

                if hi_max >= min_hist:
                    for t in range(min_hist, hi_max + 1):
                        window_len = t + 1  # seq[:t+1] 的长度
                        # 窗口需要够长：n_target 个 target + 至少 1 个 history
                        if window_len >= n_target + 1:
                            datasets['train']['user'].append(user)
