...
class DecoderBlock(nn.Module):
    ...
    def forward(self, x, encoder_hidden=None, attention_mask=None, 
        ...
        attn_output, present_key_value = self.self_attn(
            self.ln_1(x),
            attention_mask=attention_mask,  # block-causal mask（None时退化为全双向）
...
class DIFF_GRM_BLOCK(AbstractModel):
    ...
    def __init__(
        ...
        # ==== Block Diffusion 参数 ====
        self.block_diffusion = config.get('block_diffusion', False)
        self.block_size = config.get('block_size', self.n_digit)  # 默认整体作为1个block（退化为原版）
        assert self.n_digit % self.block_size == 0, \
            f"n_digit({self.n_digit}) must be divisible by block_size({self.block_size})"
        self.n_blocks = self.n_digit // self.block_size
        if self.block_diffusion:
            print(f"[MODEL] ▶ Block Diffusion enabled: n_digit={self.n_digit}, "
                  f"block_size={self.block_size}, n_blocks={self.n_blocks}, augment_factor={self.n_blocks}")

        # ==== 读取新策略 ====
        self.masking_strategy = config.get('masking_strategy', 'random')  # random | sequential | block
        ...
        # Block diffusion 覆盖 augment_factor（必须在 masking_strategy 初始化之后）
        if self.block_diffusion:
            self.augment_factor = self.n_blocks
    ...
    def _build_block_causal_mask(self, device):
        """
        Build block-causal attention mask for decoder self-attention.

        Within the same block: bidirectional (can see each other).
        Across blocks: causal (block k can see blocks 0..k-1, not k+1..n_blocks-1).

        Example: n_digit=4, block_size=2  →  block_id = [0,0,1,1]

              d0    d1    d2    d3
        d0 [True  True  False False]   block0 can only see block0
        d1 [True  True  False False]
        d2 [True  True  True  True ]   block1 can see block0 + block1
        d3 [True  True  True  True ]

        Returns:
            mask: [n_digit, n_digit] bool tensor, True = allowed to attend
        """
        n = self.n_digit
        bs = self.block_size
        ...
        for i in range(n):
            ...
            for j in range(n):
                block_j = j // bs
                if block_j <= block_i:   # j is in earlier or same block
                    mask[i, j] = True
        return mask  # [n_digit, n_digit]
    ...
    def forward(self, batch: dict, return_loss=True) -> ModelOutput:
        ...
        if self.block_diffusion:
            ...
            # 确定性生成 n_blocks 个 view，每个 view 对应一个 block_t：
            #   block_t=0: 所有 block 都 mask（全掩）
            #   block_t=1: block 0 给 ground truth，block 1 及之后 mask
            #   ...
            for block_t in range(self.n_blocks):
                mask_pos = torch.zeros(B, self.n_digit, dtype=torch.bool, device=device)
                start = block_t * self.block_size
                mask_pos[:, start:] = True  # block_t 及之后全掩

                masked_input = decoder_input_ids.clone()
                masked_input[mask_pos] = 0

                all_masked_input_ids.append(masked_input)
                all_labels.append(decoder_labels)
                all_mask_positions.append(mask_pos.float())
                all_encoder_hidden.append(encoder_hidden)

        elif self.masking_strategy == 'sequential':
            # 连贯多视图：支持多路径并行
        ...
        # Build block-causal attention mask for decoder self-attention
        if self.block_diffusion:
            block_causal_mask = self._build_block_causal_mask(device)  # [n_digit, n_digit]
            # Expand to [B_expanded, 1, n_digit, n_digit] for MultiHeadAttention
            dec_attn_mask = block_causal_mask.unsqueeze(0).unsqueeze(0).expand(
                B_expanded, 1, self.n_digit, self.n_digit
            )
        else:
            dec_attn_mask = None
        ...
        for i, block in enumerate(self.decoder_blocks):
            block_output = block(
                ...
                attention_mask=dec_attn_mask,      # block-causal mask（None时退化为全双向）
            )
