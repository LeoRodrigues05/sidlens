...
class DIFF_GRM(AbstractModel):
    ...
    def __init__(
        ...
        # Multi-item target support
        self.n_target_items = config.get('n_target_items', 1)
        self.n_target_digits = self.n_target_items * self.n_digit  # decoder 序列总长度
        if self.n_target_items > 1:
            print(f"[MODEL] ▶ Multi-item target: n_target_items={self.n_target_items}, "
        ...
        if self.masking_strategy == 'sequential':
            # 连贯多视图（步数基于 n_target_digits）
            seq_cfg = config.get('sequential_steps', 'auto')
            self.seq_steps = self.n_target_digits if seq_cfg in (None, 'auto') else int(seq_cfg)
            assert 1 <= self.seq_steps <= self.n_target_digits, \
                f"sequential_steps must be 1~{self.n_target_digits}, got {self.seq_steps}"
            ...
            # 置信度引导的连贯多视图（步数基于 n_target_digits）
            guided_cfg = config.get('guided_steps', 'auto')
            self.guided_steps = self.n_target_digits if guided_cfg in (None, 'auto') else int(guided_cfg)
            # 限制最多 n_target_digits 步
            self.guided_steps = min(self.guided_steps, self.n_target_digits)
        ...
        # 新增：掩码嵌入表，用于表示被掩码的位置（长度=n_target_digits）
        self.mask_emb_table = nn.Embedding(self.n_target_digits, self.n_embd)
    ...
    def _compute_digit_logits(self, hidden_last, digit):
        ...
        Args:
            ...
            digit: 0..n_target_digits-1 - decoder 序列中的位置
        ...
        if digit >= self.n_target_digits:
            raise ValueError(f"digit={digit} 超出范围，应该在 [0, {self.n_target_digits-1}]")

        # 循环复用 codebook：位置 4 → codebook 0，位置 5 → codebook 1 ...
        actual_digit = digit % self.n_digit

        # 2.1 取出 embedding matrix 的相应切片
        # token ID 布局 = [PAD, BOS, EOS, digit0 256 个, digit1 256 个, ...]
        start = self.tokenizer.sid_offset + actual_digit * self.codebook_size
    ...
    def forward(self, batch: dict, return_loss=True) -> ModelOutput:
        ...
        if not bool(valid_hist):
            print(f"OUT_OF_BOUNDS: {history_sid[(history_sid != -1) & ((history_sid < 0) | (history_sid >= self.codebook_size))]}")
        ...
        # --- 多概率掩码扩展 ---
        # 注意：decoder 维度用 n_target_digits（= n_target_items * n_digit）
        D = self.n_target_digits  # decoder 序列长度
        decoder_input_ids = batch['decoder_input_ids'].to(device)  # [B, D]
        decoder_labels = batch['decoder_labels'].to(device)  # [B, D]
        ...
        if self.masking_strategy == 'sequential':
            ...
            for p in range(self.sequential_paths):
                orders = torch.argsort(torch.rand(B, D, device=device), dim=1)

                full_mask = torch.ones(B, D, dtype=torch.bool, device=device)
                inp0 = decoder_input_ids.new_zeros(B, D)
            ...
            def score_with_mask(cur_mask: torch.Tensor):
                cur_inp = decoder_input_ids.new_zeros(B, D)
            ...
            if not refresh:
                ...
                for t in range(1, self.guided_steps + 1):
                    cur_mask = torch.zeros(B, D, dtype=torch.bool, device=device)
                    cols = order[:, :t]
            ...
            for view_idx, mask_prob in enumerate(self.mask_probs):
                ...
                mask_positions = torch.rand(B, D, device=device) < mask_prob
        ...
        # 合并所有视图：[B*n_views, ...]
        decoder_input_ids = torch.cat(all_masked_input_ids, dim=0)   # [B*n_views, D]
        decoder_labels = torch.cat(all_labels, dim=0)                # [B*n_views, D]
        mask_positions = torch.cat(all_mask_positions, dim=0)        # [B*n_views, D]
        encoder_hidden = torch.cat(all_encoder_hidden, dim=0)

        # 更新batch大小并验证形状
        B_expanded = B * self.augment_factor

        assert decoder_input_ids.shape[0] == B_expanded
        assert decoder_labels.shape[0] == B_expanded
        assert mask_positions.shape[0] == B_expanded
        assert encoder_hidden.shape[0] == B_expanded

        # 一致性检查：guided策略应该逐步增加掩码数
        if self.masking_strategy == 'guided':
            m = mask_positions.view(B, self.augment_factor, D).sum(-1)
        ...
        # 构建decoder输入嵌入（长度 D = n_target_digits）
        decoder_emb = torch.zeros(B_expanded, D, self.n_embd, device=device)

        for d in range(D):
            codebook_ids = decoder_input_ids[:, d]
            # 循环复用 codebook：位置 d 对应 codebook (d % n_digit)
            actual_d = d % self.n_digit
            token_ids = codebook_ids + self.tokenizer.sid_offset + actual_d * self.codebook_size
            token_ids = torch.clamp(token_ids, 0, self.vocab_size - 1)
        ...
        decoder_hidden = self.ln_f(decoder_hidden)  # [B_expanded, D, emb_dim]
    ...
    def forward_decoder_only(self, batch: dict, return_loss=False, digit=None, 
        ...
        # 构建decoder输入嵌入（长度 = n_target_digits）
        D = n_digit  # n_digit here is from decoder_input_ids.shape[1], equals n_target_digits
        decoder_emb = torch.zeros(B, D, self.n_embd, device=device)

        for d in range(D):
            # 循环复用 codebook
            actual_d = d % self.n_digit
            token_ids = decoder_input_ids[:, d] + self.tokenizer.sid_offset + actual_d * self.codebook_size
            token_ids = torch.clamp(token_ids, 0, self.vocab_size - 1)
    ...
    def generate(self, batch, n_return_sequences=1, mode="confidence", return_scores=False):
        """
        使用向量化迭代式掩码填充进行推理生成
        ...
        Args:
            ...
            return_scores: 是否同时返回 logprob 分数

        Returns:
            如果 return_scores=False: generated_sequences [B, top_k_final, n_digit]
            如果 return_scores=True:  (generated_sequences, logprobs) 元组
        """
        ...
        try:
            ...
            with torch.no_grad():
                ...
                if mode in ("confidence", "random"):
                    result = fast_beam_search_for_eval(
                        model=self,
                        encoder_hidden=encoder_hidden,
                        beam_size=n_return_sequences,
                        max_len=self.n_target_digits,
                ...
                if mode.startswith("confidence_s") and bool(self.config.get('ablate_decode', {}).get('enabled', False)):
                    ...
                    if steps >= 4:
                        ...
                        result = decode_ablate_confidence(
                            model=self,
                            encoder_hidden=encoder_hidden,
                            tokenizer=self.tokenizer,
                            steps=steps,
