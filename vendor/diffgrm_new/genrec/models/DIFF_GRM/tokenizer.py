...
class DIFF_GRMTokenizer(AbstractTokenizer):
    ...
    def __init__(self, config: dict, dataset: AbstractDataset):
        ...
        # 选择量化器：opq_pq(默认) | rq_kmeans | none(随机) | external(外部sem_ids)
        self.sid_quantizer = config.get('sid_quantizer', 'opq_pq')
        assert self.sid_quantizer in ('opq_pq', 'rq_kmeans', 'none', 'external'), \
            f"sid_quantizer must be one of ['opq_pq','rq_kmeans','none','external'], got {self.sid_quantizer}"
        ...
        elif self.sid_quantizer == 'external':
            self.index_factory = f'EXTERNAL{config["n_digit"]}x{self.n_codebook_bits}'
        else:  # 'none'
    ...
    def _init_tokenizer(self, dataset: AbstractDataset):
        ...
        # external 模式：直接加载外部 sem_ids 文件，跳过所有生成步骤
        if self.sid_quantizer == 'external':
            external_name = self.config.get('external_sem_ids', '')
            sem_ids_path = os.path.join(cache_dir, f'{external_name}.sem_ids')
            self.log(f'[TOKENIZER] Loading external semantic IDs from {sem_ids_path}...')
            item2sem_ids = json.load(open(sem_ids_path, 'r'))
            item2tokens = self._sem_ids_to_tokens(item2sem_ids)

            map_tag = f'EXTERNAL{self.config["n_digit"]}x{self.n_codebook_bits}_{external_name}_{self.n_digit}d'
        else:
            # 非 external 模式：原有逻辑（生成或加载 sem_ids）
            ...
            sent_embs = None
            need_regenerate_sem_ids = force_regenerate or not os.path.exists(sem_ids_path)

            if need_regenerate_sem_ids:
                if self.sid_quantizer == 'opq_pq':
        ...
        # 🚀 修复①：处理映射文件的一致性
        force_regenerate = self.config.get('force_regenerate_opq', False) if self.sid_quantizer != 'external' else False
        if force_regenerate:
    ...
    def _save_mappings(self):
        ...
        if self.sid_quantizer == 'rq_kmeans':
            ...
            # Without this the tag is just EXTERNAL{n}x{bits}, which is identical
            # for rqvae/rqkmeans/MQ at the same digit count and codebook size, so
            # every method writes the same two files. Nothing reads them back -
            # the load tag above is a different format and never matches - but
            # two variants running at once would still race on the write.
            quant_tag += f'_{self.config.get("external_sem_ids", "")}'
        map_tag = f'{model_basename}_pca{self.config["sent_emb_pca"]}_{quant_tag}_{self.n_digit}d'
    ...
    def tokenize_function(self, example: dict, split: str) -> dict:
        """tokenize函数 - 支持 n_target_items 个 target"""
        item_seq = example['item_seq']  # Python list
        n_target_items = self.config.get('n_target_items', 1)

        # history = 除最后 n_target_items 个之外的所有 item
        history_sid, history_mask = self.encode_history_with_mask(item_seq[:-n_target_items])

        # 编码 n_target_items 个 target，拼接 codebook IDs
        target_items = item_seq[-n_target_items:]  # 最后 n 个 item
        decoder_input_all = []
        decoder_labels_all = []
        for t_item in target_items:
            cb_input, cb_labels = self.encode_decoder_input(t_item)
            decoder_input_all.extend(cb_input)
            decoder_labels_all.extend(cb_labels)
        # decoder_input_all 长度 = n_target_items * n_digit

        if split == 'train':
            return {
                'history_sid': history_sid,
                'history_mask': history_mask,
                'decoder_input_ids': decoder_input_all,
                'decoder_labels': decoder_labels_all
            }
        else:
            return {
                'history_sid': history_sid,
                'history_mask': history_mask,
                'labels': decoder_labels_all
            }
