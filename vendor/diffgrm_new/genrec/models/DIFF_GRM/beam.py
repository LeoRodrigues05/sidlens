...
def iterative_mask_decode(model, encoder_hidden, n_return_sequences=1, tokenizer=None, mode="confidence", rand_cfg=None):
    ...
    with torch.no_grad():
        ...
        if dedup_strategy == "none":
            ...
            final_logprobs = top_logprobs  # [B, TOP_K_FINAL]
            ...
            for b in range(batch_size):
                ...
                for idx in sorted_indices:
                    ...
                    if not is_duplicate:
                        ...
                        unique_lp.append(batch_logprobs[idx])
                ...
                while len(unique_sequences) < TOP_K_FINAL:
                    if unique_sequences:
                        ...
                        if not unique_sequences:
                            ...
                            unique_lp.append(torch.tensor(NEG_INF_FP32, device=device))

                batch_final = torch.stack(unique_sequences[:TOP_K_FINAL])
                batch_lp_final = torch.stack(unique_lp[:TOP_K_FINAL])
                final_sequences.append(batch_final)
                final_logprobs.append(batch_lp_final)
    ...
    if tokenizer is not None:  # 不再限制 batch_size==1
        ...
        return final_sequences, final_logprobs, final_legal_ratio, duplicate_ratio
    # --------------------------------
    
    return final_sequences


def fast_beam_search_for_eval(model, encoder_hidden, beam_size=10, max_len=4, tokenizer=None, mode="confidence", rand_cfg=None, return_scores=False):
    """
    ...
    Returns:
        如果 return_scores=False: torch.Tensor [batch_size, TOP_K_FINAL, max_len]
        如果 return_scores=True:  (sequences, logprobs) 元组
    """
    ...
    if isinstance(result, tuple):
        sequences, logprobs = result[0], result[1]
        if return_scores:
