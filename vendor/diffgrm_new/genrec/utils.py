...
def get_file_name(config: dict, suffix: str = ''):
    ...
    if time_str != 'unknown':
        ...
        logfilename = "{}_{}{}_2items1block".format(dataset_name, current_time, suffix)
...
def get_model(model_name: Union[str, AbstractModel]) -> AbstractModel:
    ...
    # 先尝试从子包 __init__ 里找（避免子包目录名覆盖 genrec.models 里的类名）
    try:
        model_class = getattr(
            importlib.import_module(f'genrec.models.{model_name}'),
            model_name
        )
    ...
    try:
        ...
        if not callable(model_class):
            raise AttributeError(f'{model_name} is not callable')
    except:
