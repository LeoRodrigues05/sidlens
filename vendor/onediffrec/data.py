    ...
    def get_history(self, row):
        ...
        for i in range(L):
            if i == 0:
                ...
                history += ",\t\"" + row['history_item_title'][i] + "\""      
        item_title_raw = str(row['item_title'])
        if '|||' in item_title_raw:
            titles = [t.strip() for t in item_title_raw.split('|||')]
            target_item = ", ".join([f'"{t}"' for t in titles]) + "\n"
        else:
            target_item = "\"" + item_title_raw + "\"\n"
        target_item_id = str(row["item_id"])
        last_history_item_id = str(eval(row["history_item_id"])[-1])
...
class SidItemFeatDataset(Dataset):
    def __init__(self, item_file, index_file, tokenizer=None, max_len=2048, sample=-1, test=False, seed=0, category=""):
        ...
        for item_id, sids in self.indices.items():
            # item_id = self.new_id_to_old_id.get(item_id)
            if item_id in self.item_feat:
                title = self.item_feat[item_id]['title']
                # Use the complete SID so 3/4/5-codebook datasets stay aligned.
                if sids:
...
class FusionSeqRecDataset(Dataset):
    ...
    def get_history(self, row):
        ...
        target_sid_raw = str(row['item_sid'])

        if '|||' in target_sid_raw:
            target_sids = [s.strip() for s in target_sid_raw.split('|||')]
        else:
            target_sids = [target_sid_raw]

        # Look up titles and descriptions for each target
        target_titles = []
        target_descriptions = []
        for sid in target_sids:
            if sid in self.sid2title:
                target_titles.append(self.sid2title[sid])
            else:
                target_titles.append(sid)

            if sid in self.sid2description:
                desc = self.sid2description[sid]
                ...
                target_descriptions.append(f"An item with semantic ID {sid}")

        target_title = ", ".join([f'"{t}"' for t in target_titles]) if len(target_titles) > 1 else target_titles[0]
        target_description = " ".join(target_descriptions)
        target_sid = " ".join(target_sids) if len(target_sids) > 1 else target_sids[0]

        # Check for deduplication
        last_history_sid = history_item_sid[-1] if history_item_sid else None
        is_duplicate = target_sids[0] == last_history_sid
...
### Response:\n{data_point["output"]}"""
    ...
    def get_history(self, row):
        ...
        item_id_raw = str(row['item_id'])
        if '|||' in item_id_raw:
            item_ids = [s.strip() for s in item_id_raw.split('|||')]
            target_sids = []
            for iid in item_ids:
                if iid in self.id2sid:
                    target_sids.append(self.id2sid[iid])
                else:
                    target_sids.append(iid)
            target_sid = "".join(target_sids)
        else:
            if item_id_raw in self.id2sid:
                target_sid = self.id2sid[item_id_raw]
            else:
                target_sid = item_id_raw
        ...
        if self.dedup and 'history_item_id' in row:
            try:
                ...
                first_target_id = item_id_raw.split('|||')[0].strip() if '|||' in item_id_raw else item_id_raw
                is_duplicate = first_target_id == last_history_item_id
            except:
                is_duplicate = False
