...
class TokenExtender:
    def __init__(self, data_path, dataset, index_file=".rqkmeans.index_3codebook_256.json"):
...
def train(
    ...
    sid_index_path: str = "",
    item_meta_path: str = "",
    # Keeping two checkpoints leaves a fallback when a run is killed mid-save,
    # which matters when long sweeps are interrupted and resumed by the scheduler.
    save_total_limit: int = 2,
    early_stopping_patience: int = 3,
):
    set_seed(seed)
    os.environ['WANDB_PROJECT'] = wandb_project
    category_dict = {"Industrial_and_Scientific": "industrial and scientific items", "Office": "office products", "Toys_and_Games": "toys and games", "Sports": "sports and outdoors", "Books": "books", "Mercari": "Mercari"}
    print(category)
    ...
    if sid_index_path and os.path.exists(sid_index_path):
        ...
        # 直接用完整路径，从文件名推断 index_file 后缀
        _dir = os.path.dirname(sid_index_path)
        _base = os.path.basename(sid_index_path)
        _dataset = _base.split('.')[0]
        _index_file = '.' + '.'.join(_base.split('.')[1:])  # e.g. ".index_3codebook_256.json"
