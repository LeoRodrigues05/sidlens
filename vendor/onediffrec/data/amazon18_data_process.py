...
def create_item_time_features_amazon18_style(metadata, item2index, id_title, reviews):
    """Create item features like amazon18_data_process"""
    item2feature = collections.defaultdict(dict)
    ...
    # Create a mapping from item_asin to list of interaction times
    item2times = collections.defaultdict(list)
    for review in reviews:
        item_asin = review['asin']
        timestamp = int(review['unixReviewTime'])
        item2times[item_asin].append(timestamp)
    
    # Sort interaction times for each item
    for item_asin in item2times:
        item2times[item_asin].sort()
    ...
    for item_asin, item_id in item2index.items():
        if item_asin in asin_to_meta:
            ...
            # Get interaction times for this item
            inter_timelist = item2times.get(item_asin, [])
...
def parse_args():
    ...
    parser.add_argument('--dataset', type=str, default='Scientific', help='Instruments / Arts / Games / Sports')
    parser.add_argument('--user_k', type=int, default=1, help='user k-core filtering')
    parser.add_argument('--item_k', type=int, default=1, help='item k-core filtering')
    parser.add_argument('--st_year', type=int, default=2016, help='start year')
    ...
    parser.add_argument('--output_path', type=str, default='../data/Amazon18/Industrial_and_Scientific', help='output directory')
...
if __name__ == '__main__':
    ...
    for i, part in enumerate(result):
        print(f"Result 第 {i} 个返回值：")
        if isinstance(part, list) and part:  # 非空列表
            print("  列表的第一个元素：", part[0])
        elif isinstance(part, dict) and part:  # 非空字典
            first_key = next(iter(part))
            print("  字典的第一个键值对：", first_key, "=>", part[first_key])
        else:
            print("  不是列表/字典，或为空：", part)
    ...
    # print(user2items) # 6292: [906, 907, 908, 909, 910, 911], 6293: [906, 907, 908, 909, 910, 911, 1222]
    ...
    #['A3L1VJMHFWONCB', ['B0038YY2HM', 'B0038YY2HM', 'B0038YY3LW'], 'B0050GW7KU', [252, 252, 608], 240, ['Loos Cableware SL2-3P 50 Piece Zinc Plated Copper Crimping Sleeve Set for 3/32" Diameter Wire Rope', 'Loos Cableware SL2-3P 50 Piece Zinc Plated Copper Crimping Sleeve Set for 3/32" Diameter Wire Rope', 'Loos Cableware AN100-C4 Stainless Steel Thimble for 3/32" and 1/8" Diameter Wire Rope (10 Piece)'], 'Break-Free CLP-4 Cleaner Lubricant Preservative Squeeze Bottle (4 -Fluid Ounce)', [4.0, 4.0, 4.0], 5.0, [1380758400, 1380758400, 1380758400], 1380758400]
    ...
    # item2time_feature = create_item_time_features_amazon18_style(metadata, item2index, id_title, filtered_reviews)
    # item2index:
    # B0002YTLFE 0
    # B000BQKBCK 1
    # id_title:
    # 0176496920 Turning Technologies Response Card (RCRF-03)
    # 0692782109 R-Cat 692782109 EKG Badge with Arrhythmia Pocket Booklet

    # result = []
    # counter = 0
    # for key, value in item2time_feature.items():
    #     for time in value['inter_time']:
    #         result.append([key, time, counter])
    #         counter += 1
    # item2new_item = np.array(result)
    ...
    # write_json_file(item2time_feature, os.path.join(args.output_path, args.dataset, f'{args.dataset}.item_time.json'))
