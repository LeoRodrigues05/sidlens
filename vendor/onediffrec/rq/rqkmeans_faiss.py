...
def unpack_faiss_codes(packed, M, nbits):
    """Turn faiss's packed byte rows into (n, M) per-level codes.

    compute_codes() returns (n, ceil(M*nbits/8)) uint8 rows holding the M codes
    as a little-endian bit stream of nbits bits each, not one code per byte.
    Those coincide only when nbits == 8, i.e. codebook_size == 256; at 128 the
    values come out scrambled and at 512 the row is even the wrong width.
    """
    ...
    if packed.shape[1] != width:
        raise ValueError(
            f"expected {width} packed bytes for M={M}, nbits={nbits}, "
            f"got {packed.shape[1]}")
    stream = np.zeros(packed.shape[0], dtype=object)
    for i in range(width):
        stream = stream | (packed[:, i].astype(object) << (8 * i))
    mask = (1 << nbits) - 1
    codes = np.stack([((stream >> (nbits * j)) & mask) for j in range(M)], axis=1)
...
def encode_with_rq(rq, data, verbose=True):
    ...
    codes = unpack_faiss_codes(rq.compute_codes(data), rq.M, get_first_nbits(rq))
    if verbose:
        print(f"  done, codes.shape={codes.shape}, max={codes.max()}\n")
    return codes
...
def save_indices_json(codes, path, use_prefix=True):
    tpl = [f"<{chr(ord('a') + i)}_{{}}>" for i in range(len(codes[0]))]
    idx = {}
...
def main():
    ...
    data_path = f"../data/Amazon18/Industrial_and_Scientific/{args.dataset}.emb-qwen-td.npy"
    out_dir = f"../data/Amazon18/Industrial_and_Scientific"
    os.makedirs(out_dir, exist_ok=True)
    out_json = os.path.join(out_dir, f"{args.dataset}.rqkmeans.index_{args.num_levels}codebook_{args.codebook_size}.json")
