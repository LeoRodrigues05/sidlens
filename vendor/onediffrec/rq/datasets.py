import numpy as np
import torch
import torch.utils.data as data


class EmbDataset(data.Dataset):

    def __init__(self, data_path):

        self.data_path = data_path
        # self.embeddings = np.fromfile(data_path, dtype=np.float32).reshape(16859,-1)
        self.embeddings = np.load(data_path)
        
        # Check for NaN values and handle them
        nan_mask = np.isnan(self.embeddings)
        if nan_mask.any():
            print(f"Warning: Found {nan_mask.sum()} NaN values in embeddings")
            # Replace NaN with zeros
            self.embeddings[nan_mask] = 0.0
            
        # Check for infinite values
        inf_mask = np.isinf(self.embeddings)
        if inf_mask.any():
            print(f"Warning: Found {inf_mask.sum()} infinite values in embeddings")
            # Replace inf with zeros
            self.embeddings[inf_mask] = 0.0
            
        print(f"Loaded embeddings shape: {self.embeddings.shape}")
        print(f"Embeddings stats - min: {self.embeddings.min():.6f}, max: {self.embeddings.max():.6f}, mean: {self.embeddings.mean():.6f}")
        
        self.dim = self.embeddings.shape[-1]

    def __getitem__(self, index):
        emb = self.embeddings[index]
        tensor_emb = torch.FloatTensor(emb)
        return tensor_emb

    def __len__(self):
        return len(self.embeddings)


class TimeEmbDataset(data.Dataset):

    def __init__(self, data_path):

        self.data_path = data_path
        # self.embeddings = np.fromfile(data_path, dtype=np.float32).reshape(16859,-1)
        
        raw_embeddings = np.load(data_path, allow_pickle=True)

        # 提取并拼接text和time embeddings
        combined_embeddings = []

        for row in raw_embeddings:
            # if row[-1] >= 1514764800:
                # continue
            text_emb = row[0]  # list
            time_emb = row[1]  # list
            tag_emb = row[2]  # list
            # 拼接成一个列表
            combined = time_emb + tag_emb
            # combined = text_emb
            # combined = text_emb + time_emb + tag_emb
            combined_embeddings.append(combined)
        
        # 转换为numpy数组，然后是tensor
        self.embeddings = np.array(combined_embeddings, dtype=np.float32)
        
        # ============ 新增：数据检查（基于转换后的数据） ============
        # Check for NaN values and handle them
        nan_mask = np.isnan(self.embeddings)
        if nan_mask.any():
            print(f"Warning: Found {nan_mask.sum()} NaN values in embeddings")
            # Replace NaN with zeros
            self.embeddings[nan_mask] = 0.0
            
        # Check for infinite values
        inf_mask = np.isinf(self.embeddings)
        if inf_mask.any():
            print(f"Warning: Found {inf_mask.sum()} infinite values in embeddings")
            # Replace inf with zeros
            self.embeddings[inf_mask] = 0.0
        
        # ============ 新增：打印统计信息 ============
        print(f"Converted embeddings shape: {self.embeddings.shape}")
        print(f"Embeddings stats - min: {self.embeddings.min():.6f}, max: {self.embeddings.max():.6f}, mean: {self.embeddings.mean():.6f}")
        
        self.dim = self.embeddings.shape[-1]
        print(f"Embedding dimension: {self.dim}")
        

    def __getitem__(self, index):
        emb = self.embeddings[index]  # 已经是拼接好的numpy数组
        tensor_emb = torch.from_numpy(emb).float()

        return tensor_emb

    def __len__(self):
        return len(self.embeddings)
