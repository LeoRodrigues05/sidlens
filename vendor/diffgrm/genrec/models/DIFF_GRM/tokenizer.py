...
class DIFF_GRMTokenizer(AbstractTokenizer):
    ...
    def _init_tokenizer(self, dataset: AbstractDataset):
        ...
        # 🚀 external 模式：直接加载外部 sem_ids 文件，跳过句向量和量化
        if self.sid_quantizer == 'external':
            ext_path = self.config['external_sem_ids']
            self.log(f'[TOKENIZER] Loading external semantic IDs from {ext_path}...')
            item2sem_ids = json.load(open(ext_path, 'r'))
            # 转为 list 格式（与内部生成的 tuple 格式一致）
            item2sem_ids = {k: list(v) for k, v in item2sem_ids.items()}
    ...
    def _save_mappings(self):
        ...
        if self.sid_quantizer == 'rq_kmeans':
            ...
            # 用外部sem_ids文件名区分不同来源
            ext_name = os.path.splitext(os.path.basename(self.config['external_sem_ids']))[0]
            quant_tag += f'_{ext_name}'
        map_tag = f'{model_basename}_pca{self.config["sent_emb_pca"]}_{quant_tag}_{self.n_digit}d'
